...
+ (NSString *)getDateStrWithDate:(NSDate *)date formatType:(FormatType)type {
    NSDateFormatter *dateFormatter = [NSDate getDateFormatterWithFormatType:type];
...
+ (NSDate *)getDateWithComponents:(NSDateComponents *)components calendar:(NSCalendar *)calendar {
    NSDate *date = [calendar dateFromComponents:components];
    return date;
}
/** 获取年月日时分秒星期*/
...
+ (NSString *)getDayNameWithDate:(NSDate *)date calendar:(NSCalendar *)calendar {
    //年月日时分秒周
    ...
    return [NSString stringWithFormat:@"%d",(int)dateComponent.day];
}
