...
/** 创建button（右边图标+标题） */
+ (UIButton *)buttonWithRightImage:(NSString *)image
                             title:(NSString *)title
                        titleColor:(UIColor *)titleColor
                              font:(UIFont *)font {
    UIButton *btn = [UIButton buttonWithType:UIButtonTypeCustom];
...
+ (UIButton *)buttonWithFrame:(CGRect)frame
                   rightImage:(NSString *)image
                        title:(NSString *)title
                   titleColor:(UIColor *)titleColor
                         font:(UIFont *)font {
    CGFloat length_width = 20;
    UIButton *btn = [UIButton buttonWithType:UIButtonTypeCustom];
    UIImage *currentImage = [UIImage imageNamed:image];
    btn.frame = frame;
    btn.width = length_width*title.length + 5 + currentImage.size.width;
    UILabel *titleL = [[UILabel alloc]initWithText:title textColor:titleColor textAlignment:NSTextAlignmentCenter font:font];
    titleL.textAlignment = NSTextAlignmentRight;
    titleL.frame = CGRectMake(0, 0, length_width*title.length, btn.height);
    [btn addSubview:titleL];
    
    UIImageView *imageV = [[UIImageView alloc]initWithImage:currentImage];
    imageV.frame = CGRectMake(btn.width - currentImage.size.width, (btn.height - currentImage.size.height)/2, currentImage.size.width, currentImage.size.height);
    [btn addSubview:imageV];
    return btn;
}
/** 创建button（右边图标+标题） */
+ (UIButton *)buttonNativationItemWithFrame:(CGRect)frame
