...
//        subDic = [XYNetworkingHandle dictionaryWithJsonData:responseObject];
        NSLog(@"-----------------------------------");
        NSLog(@"subDic = %@",subDic);
        ...
        if ([subDic isKindOfClass:[NSDictionary class]]) {
            if ([subDic[@"resultCode"] isEqualToString:@"0000"]) {
                NSString *resultDesc = [NSString safe_string:subDic[@"resultDesc"]];
                success([subDic valueForKey:@"data"],resultDesc);
            } else if ([subDic[@"resultCode"] isEqualToString:@"2004"]||[subDic[@"resultCode"] isEqualToString:@"2002"]||[subDic[@"resultCode"] isEqualToString:@"2001"]) {
                if ([UserModel sharedInstance].loginState) {
                    [[UserModel sharedInstance] signOut];
                    [MBProgressHUD showError:@"登录失效，请重新登录"];
//                    [JumpLinkModel backToMineVC];zhge
                }
                failure(ErrorTypeRequestFailed, @"登录失效，请重新登录", subDic[@"resultCode"]);
            } else if ([subDic[@"resultCode"] isEqualToString:@"9999"]) {
                NSString *resultDesc = NO_NETWORK_ALERT;
                failure(ErrorTypeRequestFailed, resultDesc, subDic[@"resultCode"]);
            } else {
                NSString *resultDesc = [NSString safe_string:subDic[@"resultDesc"]];
...
- (void)reachabilityStatusChange {
    ...
    [manager setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        switch (status) {
            ...
            {
//                if (![AdvertiseView sharedInstance].isShowAdvertise) {
//                    [UIUpdateWindow checkVersion];
//                }
                //蜂窝
