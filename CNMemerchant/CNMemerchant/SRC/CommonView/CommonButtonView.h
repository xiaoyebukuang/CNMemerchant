//
//  CommonButtonView.h
//  CNMemerchant
//
//  Created by 陈晓 on 2020/3/4.
//  Copyright © 2020 XYBK. All rights reserved.
//

#import <UIKit/UIKit.h>

NS_ASSUME_NONNULL_BEGIN

static const CGFloat common_button_height = 50.f;

typedef void(^CommonButtonViewBlock)(void);

@interface CommonButtonView : UIView

@property (nonatomic, copy) CommonButtonViewBlock callBack;

- (instancetype)initWithTitle:(NSString *)title callBack:(CommonButtonViewBlock)callBack;

- (instancetype)initBottomWithTitle:(NSString *)title callBack:(CommonButtonViewBlock)callBack;


@end

NS_ASSUME_NONNULL_END
