...
//            NSUInteger proposeLength = textField.text.length - range.length + string.length;
//            NSString *startStr = [textField.text substringToIndex:range.location];
//            NSString *endStr = [textField.text substringFromIndex:range.location + range.length];
//            NSString *allStr = [NSString stringWithFormat:@"%@%@%@",startStr,string,endStr];
//            if (proposeLength > self.maxCount) {
//                textField.text = [allStr substringToIndex:self.maxCount];
//                return NO;
//            }
        }
            break;
        case UITextFieldToolNonChinese:
        {
            ...
            break;
        case UITextFieldToolDecimalPoint:
        {
            //数字+小数点
            NSUInteger length = string.length;
            for (int loopIndex = 0; loopIndex < length; loopIndex ++) {
            ...
            //计算截取的长度
            NSArray *pointArray = [allStr componentsSeparatedByString:@"."];
