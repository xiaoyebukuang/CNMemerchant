...
- (void)getImagesFromPHAsset:(NSArray *)assetArr callBack:(void(^)(NSArray <UIImage *>* images))callBack {
    ...
    for (PHAsset *asset in assetArr) {
        [[PHImageManager defaultManager] requestImageForAsset:asset targetSize:CGSizeMake(MAIN_SCREEN_WIDTH, MAIN_SCREEN_HEIGHT) contentMode:PHImageContentModeAspectFill options:options resultHandler:^(UIImage * _Nullable result, NSDictionary * _Nullable info) {
            if (result) {
